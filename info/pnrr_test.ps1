...
# 1) keep_columns
$kc = @("python","functions\keep_columns.py", $Raw, "-o", $combined)
...
# 2) filter (merge + OT)
Run-Cmd @("py","functions\filter.py","--input",$combined,"--schemes","schemes","--output","fluency_data\filtered_snafu.csv","--min-ot-run",$MinOTRun)

# 3) analyze
$a = @("py","functions\analyze_snafu.py","--filtered-file","fluency_data\filtered_snafu.csv","--scheme-dir","schemes","--results-dir","results")
if ($Categories.Count -gt 0) { $a += @("--categories"); $a += $Categories }
if ($ForceMerge) { $a += @("--force-merge") }
Run-Cmd $a

# 4) plot
Run-Cmd @("py","functions\plot_snafu_results.py")
