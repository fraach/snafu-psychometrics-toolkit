...
Questa guida mostra esempi concreti per usare gli strumenti della repo su un dataset PNRR: `functions/keep_columns.py`, `functions/filter.py`, `functions/analyze_snafu.py` e `functions/plot_snafu_results.py`.
...
```powershell
python functions/keep_columns.py "fluency_data\Snafu_2025-10-22_10-02-58.csv" `
...
## 2) Merge + filtro OT (functions/filter.py)
Unisce eventuali token contigui e rimuove sequenze di item fuori categoria (OT) di lunghezza ≥ N.

- Dataset combinato:
```powershell
py functions/filter.py --input fluency_data\snafu_pnrr.csv --schemes schemes `
  --output fluency_data\filtered_snafu.csv --min-ot-run 3
```
- (Opzionale) Solo maschi/femmine (se creati al passo 1):
```powershell
py functions/filter.py -i fluency_data\snafu_pnrr_male.csv   -s schemes -o fluency_data\filtered_snafu_male.csv   --min-ot-run 3
py functions/filter.py -i fluency_data\snafu_pnrr_female.csv -s schemes -o fluency_data\filtered_snafu_female.csv --min-ot-run 3
```

## 3) Analisi (functions/analyze_snafu.py)
...
```powershell
py functions/analyze_snafu.py --filtered-file fluency_data\filtered_snafu.csv --categories animali frutta verdura
```
- Parametri del metodo Conceptual Network (facoltativo):
```powershell
py functions/analyze_snafu.py --filtered-file fluency_data\filtered_snafu.csv --cn-alpha 0.01 --cn-window 3 --cn-threshold 3
```

(Per ripetere il merge automatico dal raw usa `--force-merge` e imposta `--raw-file`.)

## 4) Grafici (functions/plot_snafu_results.py)
Genera i grafici delle reti e i riassunti. Il layout è già più “spazioso” e le figure si scalano con il numero di nodi.
```powershell
py functions/plot_snafu_results.py
```
...
## Troubleshooting
- Errori di encoding su CSV reti: il loader è “robusto” (utf‑8‑sig/utf‑8/cp1252/latin1). Se persiste, rigenera le reti con `functions/analyze_snafu.py`.
- `snafu` mancante: esegui `SETUP_WINDOWS.ps1` oppure `pip install snafu` (o dal repo Git se non su PyPI).
- Grafici densi: in `functions/plot_snafu_results.py` modifica `PLOT_CFG` (k‑core, layout="kamada", max_labels più basso).
